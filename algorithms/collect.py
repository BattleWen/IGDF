...
class TrainConfig:
    ...
    env_name: str = "HalfCheetah-morph"
    batch_size: int = 256
    total_timesteps: int = 1005000
...
def train(config: TrainConfig):
    ...
    while env_steps < config.total_timesteps - 1:
        done = False
        state = env.reset()
        step_env = 0  
        while not done:
            ...
            buffer.add_transition(state, action, reward, next_state, done_rb)
            state = next_state

            if buffer._size > config.buffer_size:
                buffer.pop(0)
            ...
            if env_steps >= config.learning_starts:
                ...
                if total_updates % config.log_every == 0:
                    wandb.log({"epoch": env_steps // 1000, **update_info}, step = total_updates)
                total_updates += 1

            # evaluation
            if env_steps % config.eval_every == 0 or env_steps == config.total_timesteps - 1:
                ...
                if config.checkpoints_path is not None:
                    torch.save(
                        trainer.state_dict(),
                        os.path.join(config.checkpoints_path, f"{env_steps}.pt"),
                    )

    results = {
        'observations': buffer._states.cpu().numpy(),
        'next_observations': buffer._next_states.cpu().numpy(),
        'actions': buffer._actions.cpu().numpy(),
        'rewards': buffer._rewards.cpu().numpy(),
        'terminals': buffer._dones.cpu().numpy(),
    }
    if config.env_name == "HalfCheetah-morph":
        os.makedirs(f"datasets/source_halfcheetah/56/", exist_ok=True)
        hfile = h5py.File("datasets/source_halfcheetah/56/buffer.hdf5", 'w')
    if config.env_name == "HalfCheetah-morph2":
        os.makedirs(f"datasets/source_halfcheetah/51/", exist_ok=True)
        hfile = h5py.File("datasets/source_halfcheetah/51/buffer.hdf5", 'w')
    if config.env_name == "Hopper-morph":
        os.makedirs(f"datasets/source_hopper/56/", exist_ok=True)
        hfile = h5py.File("datasets/source_hopper/56/buffer.hdf5", 'w')
    if config.env_name == "Hopper-morph2":
        os.makedirs(f"datasets/source_hopper/51/", exist_ok=True)
        hfile = h5py.File("datasets/source_hopper/51/buffer.hdf5", 'w')
    if config.env_name == "Walker-morph":
        os.makedirs(f"datasets/source_walker2d/56/", exist_ok=True)
        hfile = h5py.File("datasets/source_walker2d/56/buffer.hdf5", 'w')
    if config.env_name == "Walker-morph2":
        os.makedirs(f"datasets/source_walker2d/51/", exist_ok=True)
        hfile = h5py.File("datasets/source_walker2d/51/buffer.hdf5", 'w')
