...
def train(config: TrainConfig):
    ...
    disc_optimizer = torch.optim.Adam(disc.parameters(), lr=config.info_lr)
    ...
    for online_step in trange(config.max_timesteps, desc='Training'):
        ...
        if total_steps % config.disc_train_freq == 0 and source_buffer._size > config.disc_batch_size:
            ...
            wandb.log(
                {"disc_loss": loss_disc,
                 "disc_loss_sas": loss_sas_disc,
                 "disc_loss_sa": loss_sa_disc,
                },
                step = total_steps
            )
        # train iql
        ...
        if total_steps > config.start_using_source:
            ...
            mask                        = torch.ones((config.batch_size * 2, 1)).to(config.device)
            mask[:config.batch_size]    = h2o_weight
            # merge data
