...
class TrainConfig:
    ...
    # SRPO  config from https://github.com/AIDefender/SRPO/blob/master/offlinerl/config/algo/maple_config_st.py 
    # and page 9 of the paper
    disc_lr: float = 1e-3
    batch_ratio: float = 0.5 # or 0.2
    lamda: float = 0.1 # or 0.3 

    def __post_init__(self):
        self.name = f"{self.name}-{str(uuid.uuid4())[:8]}-{self.seed}-{self.batch_ratio}-{self.lamda}"
...
class StateDiscriminator(nn.Module):
    ...
    def forward(self, s: torch.Tensor) -> torch.Tensor:
        return self.net(s)      #   logits of P(real|s)
...
class StateRegularizedImplicitQLearning:
    def __init__(
        self,
        max_action: float,
        actor: nn.Module,
        ...
        # srpo config
        disc_network: nn.Module,
        disc_optimizer: nn.Module,
        batch_ratio: float,
        lamda: float,
        # # # #
        iql_tau: float = 0.7,
    ...
    def _update_disc(
        ...
        # evaluate the observations via V function
        with torch.no_grad():
            value_of_s      =   self.vf(observations)
        # regard the obs with higher values as real data, others as fake
        sorted_indices      =   torch.argsort(value_of_s[:])     # ascending
        real_indices        =   sorted_indices[-int(len(observations) * self.batch_ratio):]
        fake_indices        =   sorted_indices[:-int(len(observations) * self.batch_ratio)]
        real_s              =   observations[real_indices]
        fake_s              =   observations[fake_indices]
        # train the discriminator via CE
        label               =   torch.ones(size=(observations.shape[0],), device=self.device)
        label[fake_indices] =   0
        label = label.unsqueeze(1)
        logits              =   self.disc(observations)
        loss_disc           =   F.binary_cross_entropy_with_logits(logits, label)
        self.disc_optimizer.zero_grad()
        loss_disc.backward()
        self.disc_optimizer.step()
        log_dict['disc_loss']   =   loss_disc.item()

    def _correct_reward(self, observations: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            real_logits      =   self.disc(observations)        #   [batch_size, 1]
        reward_aug  =   torch.log(torch.clamp(real_logits, 0.1, 10))    #   from https://github.com/AIDefender/SRPO/blob/master/offlinerl/algo/modelbase/maple_st.py/ line 443
        return reward_aug   #   [batch_size, 1]
    ...
    def train(self, batch: TensorBatch) -> Dict[str, float]:
        ...
        self._update_disc(observations, log_dict)
        # augment the reward
        rew_aug = self._correct_reward(observations)
        assert rewards.shape == rew_aug.shape
        rewards = rewards + self.lamda * rew_aug
...
def train(config: TrainConfig):
    ...
    target_data["observations"]         = normalize_states(target_data["observations"], state_mean, state_std)
    target_data["next_observations"]    = normalize_states(target_data["next_observations"], state_mean, state_std)
    source_data["observations"]         = normalize_states(source_data["observations"], state_mean, state_std)
    source_data["next_observations"]    = normalize_states(source_data["next_observations"], state_mean, state_std)
    env     = wrap_env(env, state_mean=state_mean, state_std=state_std)

    # target/source replay buffer
    target_buffer = ReplayBuffer(state_dim, action_dim, config.buffer_size, config.device)
    source_buffer = ReplayBuffer(state_dim, action_dim, config.buffer_size, config.device)
    ...
    disc_network    = StateDiscriminator(state_dim, ).to(config.device)
    ...
    disc_optimizer  = torch.optim.Adam(disc_network.parameters(), lr=config.disc_lr)
    ...
    # Initialize actor
    trainer = StateRegularizedImplicitQLearning(**kwargs)
